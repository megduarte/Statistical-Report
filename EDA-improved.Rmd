...
TFR_train_forecast |> autoplot() +
  ...
  labs(
    ...
    caption = stringr::str_wrap(
      "Figure 13. Plot shows that the predictive model drops more than the actual data does for the 2013-2024 period for the Total Fertility Rate in Singapore." ,width = 80
    )
  )+
theme(
...
TLB_train_forecast |> autoplot() +
  ...
  labs(
    ...
    caption = stringr::str_wrap(
      "Figure 14. Plot shows that the predictive model drops more than the actual data does for the 2013-2024 period for the Total Live Births in Singapore." ,width = 80
    )
  )+
theme(
  plot.title = element_text(hjust = 0.5),
...
TFR_train_model2 <- TFR_Train |>
  model(tfr_linear = TSLM(log_TFR ~ trend()))
...
TLB_train_model2 <- TLB_Train |>
  model(tlm_linear = TSLM(log_TLB ~ trend()))
...
TFR_train_sarima_fc |>
  ...
  labs(
    ...
    caption = stringr::str_wrap(
      "Figure 17. Plot shows that the SARIMA predictive model shows lower total fertility rates in singapore from 2013-2024 than the known data shows." ,width = 80
    )
  )+
theme(
  plot.title = element_text(hjust = 0.5),
...
Figure 17 shows that similarly to the ARIMA predictive model. the predictive model expects the total fertility rate to drop more in 2013-2024 than it actually does. However, unlike the ARIMA predictive model seen in figure 13, the SARIMA(0,1,1)(1,0,1)[12] model accounts for seasonality. In the ARIMA model, the predicted values for 2013-2024, was a straight line with a negative slope. The predictive center line for the SARIMA model has "wiggle" to it, accounting for rises and drops in the TFR. This looks much more similar to the actual data. 
...
```
Table 11. Shows the automated search selects a SARIMA(1,1,0)(1,0,0)[12] model for the Total Live-Births data.

```{r}
glance(TLB_Model2)
```

Table 12. Shows the SARIMA(1,1,0)(1,0,0)[12] model for Total Live-Births has a lower AIC at around -191, over 10 points lower when compared to the ARIMA(0,1,0) model for Total Live-Births. The BIC is also lower for the SARIMA model, and the sigma^2 value is also significantly lower for the SARIMA model. 

All of the values in table 12 confirm that a seasonal ARIMA model is better at handling the Total Live-Births data in Singapore from 1960-2024. 

```{r}
TLB_train_sarima <- TLB_Train |>
...
TLB_train_sarima_fc |>
  ...
  labs(
    ...
    caption = stringr::str_wrap(
      "Figure 18. Plot shows that the SARIMA predictive model shows lower total live-births in singapore from 2013-2024 than the known data shows." ,width = 80
    )
  )+
theme(
  plot.title = element_text(hjust = 0.5),
...
Figure 18 shows that the predicted Total Live-Births is lower than the actual Total Live-Births in Singapore from 2013-2024, however, a good portion of the actual data lies within the 80% confidence interval. Similarly to the Total Fertility-Rate figures, 17 and 13, figure 18 and the ARIMA model seen in figure 14, also demonstrates how SARIMA is a better model to pick for this singapore data set. 
...
tfr_resid |> ACF(.resid) |>
  ...
  labs(
    ...
    caption = stringr::str_wrap("Figure 19. ACF Plot of the SARIMA model shows that there is a spike only at lag 13.", width = 75)
  ) +
  theme(plot.caption = element_text(hjust = 0.5),
        plot.title = element_text(hjust = 0)
  )
...
tfr_resid |> PACF(.resid) |>
  ...
  labs(
    ...
    caption = stringr::str_wrap("Figure 20. PACF Plot of the SARIMA model shows that there is a spike at lag 15 and potentially at lag 13.", width = 75)
  ) +
  theme(plot.caption = element_text(hjust = 0.5),
        plot.title = element_text(hjust = 0)
  )
...
tlb_resid |> ACF(.resid) |>
  ...
  labs(
    ...
    caption = stringr::str_wrap("Figure 21. ACF Plot of the predictive SARIMA shows that there is a spike only at lag 13.", width = 75)
  ) +
  theme(plot.caption = element_text(hjust = 0.5),
        plot.title = element_text(hjust = 0)
  )
...
tlb_resid |> PACF(.resid) |>
  ...
  labs(
    ...
    caption = stringr::str_wrap("Figure 22. PACF Plot of the predictive SARIMA shows that there is a spike only at lag 13.", width = 75)
  ) +
  theme(plot.caption = element_text(hjust = 0.5),
        plot.title = element_text(hjust = 0)
  )
